...
use quote::quote;
use syn::{parse2, parse_macro_input, spanned::Spanned, Attribute, Expr, FieldValue, Fields, Ident, ItemStruct, Member, Token, Type, TypePath};
...
/// gera uma versão deste bloco de configurações onde todos os campos de opção marcados como `#[settings_block]` são substituídos por
/// versões desses campos que sempre persistem os valores "some". isto é útil para, por exemplo, manter o estado de
/// 'configurações' na interface do usuário, mesmo que essas configurações estejam desativadas no momento.
