...
RingPattern = np.load(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'ringPattern.py'))


def ringing(img2d, alpha=0.5, noiseSize=0, noiseValue=2, clip=True, seed=None):
    """
    https://www.avartifactatlas.com/artifacts/ringing.html
    :param img2d: imagem 2d
    :param alpha: float, qualidade de reconstrução (0-1)
    :param noiseSize: float, tamanho do noise (0-1)
    :param noiseValue: float, amplitude do noise (0-5)
    :return: imagem 2d
    """
...
def ringing2(img2d, power=4, shift=0, clip=True):
    ...
    :param img2d: imagem 2d
    :param power: int, poser de ringing
    :return: imagem 2d
    """
    dft = cv2.dft(np.float32(img2d), flags=cv2.DFT_COMPLEX_OUTPUT)
    dft_shift = np.fft.fftshift(dft)
...
# bgr hwc uint8 para -> yiq chw int32 planar
...
# um field do yiq chw int32 planar -> um field do bgr hwc uint8
...
# provavelmente o que seu crt antigo faz para reduzir fringes de cor
...
class Ntsc:
    ...
    def __init__(self, precise=False, random=None):
        ...
        self._composite_preemphasis_cut = 1000000.0 # artefatos de analog relacionados a qualquer coisa que afete o sinal cru (como a modulação catv)
        ...
        self._color_bleed_before = True # bleeding de cor vem antes de outras degradações se for Verdadeiro ou depois de outra forma
        self._color_bleed_horiz = 0 # bleeding de cor horizontal 0 = sem bleed de cor, 1..10 valores sanos
        self._color_bleed_vert = 0 # bleeding de cor vertical 0 = sem bleed de cor, 1..10 valores sanos
        self._ringing = 1.0 # 1 = sem ringing, 0.3..0.99 = valores sanos
        self._enable_ringing2 = False
        self._ringing_power = 2
        self._ringing_shift = 0
        self._freq_noise_size = 0 # sem ruído
        self._freq_noise_amplitude = 2 # amplitude de ruído (0-5)
        
        self._composite_in_chroma_lowpass = True # aplicar lowpass chroma antes da codificação composta
        ...
        self._output_ntsc = True # emulação de subportadora de cores ntsc
    ...
    def video_noise(self, yiq: numpy.ndarray, field: int, video_noise: int):
        ...
        if not self.precise: # esse trabalha rapidamente
        ...
        else: # esse trabalha exatamente como o código original
            noise = 0
    ...
    # https://www.avartifactatlas.com/artifacts/chrominance_noise.html
    ...
    def chroma_from_luma(self, yiq: numpy.ndarray, field: int, fieldno: int, subcarrier_amplitude: int):
        ...
        for y in range(field, height, 2):
            ...
            # inverter a parte da onda senoidal que corresponderia aos valores negativos de u e v
            chroma[x + 2::4] = -chroma[x + 2::4]
            chroma[x + 3::4] = -chroma[x + 3::4]
            
            chroma = (chroma * 50 / subcarrier_amplitude)

            # decodificar a cor da subportadora que foi gerado
            cxi = -chroma[xi::2]
            cxi1 = -chroma[xi + 1::2]
    ...
    # os decks vhs também mancham verticalmente a subportadora de croma usando uma linha
    # de atraso para adicionar a subportadora de cores da linha anterior à subportadora
    # de cores da linha atual.
    ...
    # https://www.avartifactatlas.com/artifacts/yc_delay_error.html
    ...
    def vhs_edge_wave(self, yiq: numpy.ndarray, field: int):
        ...
        lp = LowpassFilter(Ntsc.NTSC_RATE, self._output_vhs_tape_speed.luma_cut, 0) # nenhum propósito real para inicializá-lo com valores ntsc
...
def random_ntsc(seed=None) -> Ntsc:
    ...
    ntsc._composite_in_chroma_lowpass = rnd.random() < 0.8 # se inclina para o valor padrão
    ntsc._composite_out_chroma_lowpass = rnd.random() < 0.8 # se inclina para o valor padrão
    ntsc._composite_out_chroma_lowpass_lite = rnd.random() < 0.8 # se inclina para o valor padrão
