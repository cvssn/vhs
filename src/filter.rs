...
// multiplica dois polinomiais (coeficientes mais baixos primeiro)
...
impl TransferFunction {
        ...
        J: IntoIterator<Item = f64>
    {
        ...
        if num.len() > den.len() {
            panic!("o comprimento do numerador excede o comprimento do denominador.");
        }
    ...
    pub fn filter_signal_in_place(&self, items: &mut [f64], initial: f64, scale: f64, delay: usize) {
        ...
        for i in 0..items.len() {
            ...
            z[filter_len - 2] = (num_padded[filter_len - 1] * sample) - (self.den[filter_len - 1] * filt_sample);
...
impl TryFrom<&TransferFunction> for StateSpace {
    ...
    fn try_from(tf: &TransferFunction) -> Result<Self, Self::Error> {
        ...
        if k < 1 || tf.den.iter().all(|elem| *elem == 0.0) {
            return Err("o denominador deve ter pelo menos um elemento diferente de zero.");
        }

        if m > k {
            return Err("função de transferência inadequada. o comprimento do numerador excede o comprimento do denominador.");
        }

        // espaço de estado vazio
        if k == 0 || tf.num.len() == 0 {
            return Err("espaço de estado vazio.");
        }
