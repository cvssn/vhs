...
class Renderer(QtCore.QObject):
    ...
    def run(self):
        ...
        print(f"render temporário: {tmp_output_filename}")
        ...
        video = cv2.VideoWriter(tmp_output_filename, fourcc, self.render_data["input_video"]["orig_fps"], render_wh)
        ...
        audio_orig = (ffmpeg.input(self.render_data["input_video"]["path"]))
        video = ffmpeg.input(tmp_output_filename)
        (ffmpeg.output(video.video, audio_orig.audio, self.render_data["target_file"], shortest=None, vcodec='copy').overwrite_output().run())
